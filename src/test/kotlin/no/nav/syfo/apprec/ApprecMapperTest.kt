...
internal class ApprecMapperTest {
    ...
    internal fun `Duplicate AppRec has the same ediLoggId as the source`() {
        val apprecErrorDuplicate = createApprecError(
    ...
    internal fun `OK AppRec should get correct genDate`() {
        ...
        Assertions.assertEquals("2021-03-03T12:01:01+01:00", apprecWithMsgGenDate.get<XMLAppRec>().originalMsgId.issueDate)
    }
    ...
    internal fun `OK AppRec should get correct genDate with msgGenDate = null`() {
        ...
        Assertions.assertEquals("2021-03-03T12:02:02", apprecWithoutMsgGenDate.get<XMLAppRec>().originalMsgId.issueDate)
    }
    ...
    internal fun `OK AppRec senders appRec sender institution name to receiver organizationName`() {
        val ff = marshalAndUnmarshal(createApprec(apprec.ediloggid, apprec, ApprecStatus.OK, listOf()))
        Assertions.assertEquals(apprec.senderOrganisasjon.navn, ff.get<XMLAppRec>().sender.hcp.inst.name)
    }

    @Test
    internal fun `OK AppRec senders appRec institution id to first organization ident id`() {
        val ff = marshalAndUnmarshal(createApprec(apprec.ediloggid, apprec, ApprecStatus.OK, listOf()))
        Assertions.assertEquals(apprec.senderOrganisasjon.hovedIdent.id, ff.get<XMLAppRec>().sender.hcp.inst.id)
    }

    @Test
    internal fun `OK AppRec senders appRec institution typeId dn to first organization ident typeId dn`() {
        val ff = marshalAndUnmarshal(createApprec(apprec.ediloggid, apprec, ApprecStatus.OK, listOf()))
        Assertions.assertEquals(apprec.senderOrganisasjon.hovedIdent.typeId.beskrivelse, ff.get<XMLAppRec>().sender.hcp.inst.typeId.dn)
    }

    @Test
    internal fun `OK AppRec senders appRec institution typeId v to first organization ident typeId v`() {
        val ff = marshalAndUnmarshal(createApprec(apprec.ediloggid, apprec, ApprecStatus.OK, listOf()))
        Assertions.assertEquals(apprec.senderOrganisasjon.hovedIdent.typeId.verdi, ff.get<XMLAppRec>().sender.hcp.inst.typeId.v)
    }
